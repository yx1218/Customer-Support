...
@MultipartConfig(
        ...
        maxRequestSize = 40 * 1024 * 1024 //40Mb
)
public class TicketServlet extends HttpServlet {
    private volatile int TICKET_ID_SEQUENCE = 1;
    private Map<Integer, Ticket> ticketDatabase = new HashMap<>();//模拟数据库
    ...
    private void downloadAttachment(HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        Ticket ticket = this.getTicket(ticketIdString, response);
        if(ticket == null){
            return;
        }
    ...
    private void viewTicket(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        writer.append("<h2>Ticket #").append(ticketIdString)
        ...
        if(ticket.getNumberOfAttachments() > 0){
            ...
            for(Attachment attachment : ticket.getAttachments()){
                ...
                writer.append("<a href=\"tickets?action=download&ticketId=")
                        .append(ticketIdString).append("&attachmentName=")
    ...
    private Ticket getTicket(String ticketIdString, HttpServletResponse response) throws IOException {
        if(ticketIdString == null || ticketIdString.length() == 0){
            response.sendRedirect("tickets");
            return null;
        }
        try {
            Ticket ticket = this.ticketDatabase.get(Integer.parseInt(ticketIdString));
            if (ticket == null) {
                response.sendRedirect("tickets");
                return null;
            }
